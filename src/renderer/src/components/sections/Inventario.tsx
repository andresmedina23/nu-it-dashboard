...
export default function Inventario({ onRun, onScript, running, onAutoResponses }: Props) {
  ...
  // ─── Google Workspace (Sheets API directa) ────────────────
  const [gwStatus, setGwStatus]           = useState<{ authenticated: boolean; hasCredentials: boolean; email?: string; name?: string } | null>(null)
  const [gwSheetsCfg, setGwSheetsCfg]     = useState<{ spreadsheetId: string; sheetName: string } | null>(null)
  const [gwSheetsExpanded, setGwSheetsExpanded] = useState(false)
  const [gwSpreadsheetId, setGwSpreadsheetId] = useState('')
  const [gwSheetName, setGwSheetName]     = useState('MAC')
  ...
  useEffect(() => {
    ...
    // Google Workspace: cargar status y config de sheets
    window.electronAPI.gwStatus().then(s => {
      setGwStatus(s)
    }).catch(() => {})
    window.electronAPI.gwSheetsConfigGet().then(cfg => {
      setGwSheetsCfg(cfg)
      if (cfg.spreadsheetId) setGwSpreadsheetId(cfg.spreadsheetId)
      if (cfg.sheetName)     setGwSheetName(cfg.sheetName)
    }).catch(() => {})
  ...
  useEffect(() => {
    if (prevRunning.current && !running) {
      ...
      // Google Workspace Sheets API directa — paralelo al sync Apps Script
      if (run && run.serial && gwSheetsCfg?.spreadsheetId && gwStatus?.authenticated) {
        window.electronAPI.gwSheetsUpdateSerial({ serial: run.serial, action: run.action, status: run.status })
          .then(result => {
            const label = result.block ? `bloque ${result.block}` : ''
            const msg = result.ok
              ? `[GW] ✓ ${result.serial} actualizado ${label}`.trim()
              : `[GW] ⚠ ${result.serial}: ${result.error ?? 'no encontrado'}`
            setSyncResult(msg)
            setTimeout(() => setSyncResult(null), 5000)
            const actionLabel = run.action === 'checkin' ? 'Checkin'
              : run.action === 'checkout' ? 'Checkout'
              : `Update → ${run.status ?? run.action}`
            setSyncLog(prev => [{
              time: new Date(),
              serial: result.serial ?? run.serial,
              action: '[GW API] ' + actionLabel,
              ok: result.ok,
              block: result.block ? `col ${result.blockCol} fila ${result.row}` : undefined,
            }, ...prev].slice(0, 30))
          })
          .catch(() => {})
      }
  ...
  }, [running, sheetsCfg, gwSheetsCfg, gwStatus])
      ...
      <div className={`mb-4 rounded-xl border transition-all
        ...
        {syncExpanded && (
          <div className="px-3 pb-3 space-y-3 border-t border-[#4A1D7A]/30 pt-3">
            ...
            {/* ─── Google Workspace Sheets API panel ─────────── */}
            <div className={`mt-3 rounded-xl border transition-all ${
              gwStatus?.authenticated && gwSheetsCfg?.spreadsheetId
                ? 'bg-[#0a1a10]/60 border-[#0DBA6A]/30'
                : 'bg-[#0F001E]/60 border-[#4A1D7A]/40'
            }`}>
              <button
                onClick={() => setGwSheetsExpanded(v => !v)}
                className="w-full flex items-center justify-between px-3 py-2 text-left"
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm">🌐</span>
                  <div>
                    <span className="text-[11px] font-medium text-[#C9B3D9]/80">Google Sheets API</span>
                    {gwStatus?.authenticated && gwSheetsCfg?.spreadsheetId && (
                      <span className="ml-2 text-[10px] text-[#0DBA6A]">● Activo — via Gemini CLI</span>
                    )}
                    {gwStatus?.authenticated && !gwSheetsCfg?.spreadsheetId && (
                      <span className="ml-2 text-[10px] text-yellow-400">● Autenticado — falta Sheet ID</span>
                    )}
                    {!gwStatus?.authenticated && (
                      <span className="ml-2 text-[10px] text-[#C9B3D9]/40">● Sin token — abre Claude Code primero</span>
                    )}
                  </div>
                </div>
                <span className={`text-[10px] text-[#C9B3D9]/40 transition-transform ${gwSheetsExpanded ? 'rotate-180' : ''}`}>▾</span>
              </button>

              {gwSheetsExpanded && (
                <div className="px-3 pb-3 space-y-3 border-t border-[#4A1D7A]/30 pt-3">
                  {/* Auth status — uses Gemini CLI token, no manual credentials needed */}
                  <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border text-[10px] ${
                    gwStatus?.authenticated
                      ? 'bg-[#0DBA6A]/10 border-[#0DBA6A]/30 text-[#0DBA6A]'
                      : 'bg-[#0F001E] border-[#4A1D7A]/40 text-[#C9B3D9]/50'
                  }`}>
                    <span className="flex-shrink-0">{gwStatus?.authenticated ? '🔑' : '🔒'}</span>
                    <span>
                      {gwStatus?.authenticated
                        ? 'Conectado via Gemini CLI / Claude Code — sin configuración adicional'
                        : 'Sin token. Abre Claude Code para autenticar el Google Workspace MCP, luego recarga la app.'}
                    </span>
                  </div>

                  {/* Sheet ID */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-[#C9B3D9]/50">Spreadsheet ID</label>
                    <input
                      type="text"
                      value={gwSpreadsheetId}
                      onChange={e => setGwSpreadsheetId(e.target.value)}
                      placeholder="1dvaUz7qHg7a8..."
                      className="w-full bg-[#0a0014] border border-[#4A1D7A]/50 rounded-lg px-2 py-1.5 text-[11px] text-white font-mono placeholder-[#4A1D7A]/60 focus:outline-none focus:border-[#820AD1]"
                    />
                    <div className="flex gap-2 items-center">
                      <input
                        type="text"
                        value={gwSheetName}
                        onChange={e => setGwSheetName(e.target.value)}
                        placeholder="MAC"
                        className="w-24 bg-[#0a0014] border border-[#4A1D7A]/50 rounded-lg px-2 py-1.5 text-[11px] text-white font-mono placeholder-[#4A1D7A]/60 focus:outline-none focus:border-[#820AD1]"
                      />
                      <span className="text-[10px] text-[#C9B3D9]/40">nombre de la hoja</span>
                      <button
                        disabled={!gwSpreadsheetId}
                        onClick={async () => {
                          const cfg = { spreadsheetId: gwSpreadsheetId.trim(), sheetName: gwSheetName.trim() || 'MAC' }
                          await window.electronAPI.gwSheetsConfigSet(cfg)
                          setGwSheetsCfg(cfg)
                        }}
                        className="ml-auto px-3 py-1 rounded-lg bg-[#820AD1]/20 hover:bg-[#820AD1]/40 disabled:opacity-40 text-[11px] text-white border border-[#820AD1]/40 transition-colors"
                      >
                        Guardar
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              {STATUSES.map(s => (
                <button key={s.label} onClick={() => { setStatusPreset(s.num); if (s.num) setStatusManual('') }}
                  className={`py-3 px-3 rounded-xl text-[13px] font-medium text-center transition-all
                    ${statusPreset === s.num && s.num !== ''
                      ? 'bg-[#820AD1] text-white shadow-lg shadow-[#820AD1]/30'
                      : 'bg-[#0F001E] border border-[#4A1D7A]/70 text-[#C9B3D9]/60 hover:border-[#820AD1]/60 hover:text-white'}`}>
                  {s.label}
                </button>
              ))}
            </div>
