...
type Tab = 'recovery' | 'ztd_enroll' | 'ztd_remove' | 'admin' | 'unlock' | 'lock'
...
const TABS = [
  ...
  { id: 'lock' as Tab, label: 'Lock Mac', icon: '🔒' },
]
...
export default function JAMF({ onRun, running }: Props) {
  ...
  const handleRun = () => {
    ...
    const map: Record<Tab, [string, string[]]> = {
      ...
      lock:       ['it', ['jamf', 'computer', 'lock', s, m || 'Offboarding']],
    }
    onRun(...map[tab])
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="nu-card space-y-4">
        ...
        {(tab === 'recovery' || tab === 'ztd_enroll' || tab === 'unlock' || tab === 'lock') && (
          <FormField label="Motivo" id="jamf-motivo" value={motivo} onChange={setMotivo}
            placeholder={tab === 'ztd_enroll' ? 'Testing CO ZTD' : tab === 'unlock' ? 'unlock' : tab === 'lock' ? 'Offboarding' : 'Soporte Técnico'} />
        )}

        {tab === 'lock' && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-xs text-red-400">
            ⚠️ El comando bloqueará el Mac via MDM. En la terminal selecciona: <strong>1</strong> (robado/perdido) o <strong>2</strong> (offboarding). El PIN de 6 dígitos se mostrará en la terminal y se notificará a Slack.
          </div>
        )}
